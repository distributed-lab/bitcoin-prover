...
""")
    ...
    while index < (blocks_amount - 1025):
        ...
        with open("./app/blocks-recursive/rec/Prover.toml", "w") as f:
            ...
            f.write(f"time_idx = \"{pi_array[-(3 + 32 * merkle_root_state_len)]}\"\n\n")
            f.write(f"last_block_height = \"{int(pi_array[-(2 + 32 * merkle_root_state_len)], 16)}\"\n\n")
            f.write(f"chainwork = \"{int(pi_array[-(1 + 32 * merkle_root_state_len)], 16)}\"\n\n")
            f.write(f"verification_key = {vk}\n\n")
            f.write(f"proof = {proof}\n\n")
            f.write(f"public_inputs = {pi}\n\n")
            f.write(f"merkle_state = {to_toml_array_hashes(pi_array[-(32 * merkle_root_state_len):])}")
            f.write(nargo_toml)
