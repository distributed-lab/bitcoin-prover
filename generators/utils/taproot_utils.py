...
def get_outputs_from_inputs(tx: Transaction) -> tuple[list[int], list[Tuple[int, int, int]]]:
    outs = []
    pos = []
    cur_pos = 0
    ...
    if response.ok:
        ...
        for inp in tx_data["vin"]:
            ...
            script_pub_key_size = bytearray.fromhex(tx._get_hex_from_compact_size(len(script_pub_key)))
