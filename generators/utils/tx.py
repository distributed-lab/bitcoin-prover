...
from bitcoin.core import lx, b2x, CTransaction
from bitcoin.core.serialize import BytesIO
import requests
import hashlib
import sys

Input = namedtuple('Input', ['txid', 'vout', 'script_sig_size', 'script_sig', 'sequence'])
Output = namedtuple('Output', ['value', 'script_pub_key_size', 'script_pub_key'])
...
class Transaction:
    ...
    def _parse_from_hex(self, hex_tx: str) -> 'Transaction':
        ...
        cur_pos = 0
        self.version = int.from_bytes(raw_bytes[cur_pos:cur_pos+4], byteorder='little')
        cur_pos += 4
        ...
        for _ in range(self.input_count):
            ...
            cur_pos += 32
            vout = int.from_bytes(raw_bytes[cur_pos:cur_pos+4], byteorder='little')
            cur_pos += 4
            ...
            sequence = int.from_bytes(raw_bytes[cur_pos:cur_pos+4], byteorder='little')
            cur_pos += 4
            self.inputs.append(Input(txid, vout, script_sig_size, script_sig, sequence))
        
        self.output_count, cur_pos = self._get_compact_size(cur_pos)

        self.outputs = []
        for _ in range(self.output_count):
            value = int.from_bytes(raw_bytes[cur_pos:cur_pos+8], byteorder='little')
            cur_pos += 8
            ...
            self.outputs.append(Output(value, script_pub_key_size, script_pub_key))
        ...
        self.lock_time = int.from_bytes(raw_bytes[cur_pos:cur_pos+4], byteorder='little')
    ...
    def _parse_from_json(self, data: Dict):
        ...
        has_witness = any('witness' in inp and inp['witness'] for inp in data['inputs'])
    ...
    def to_hex(self) -> str:
        ...
        input_hex = "".join([self._get_input_hex(input) for input in self.inputs])

        output_count_hex = self._get_hex_from_compact_size(self.output_count)

        output_hex = "".join([self._get_output_hex(output) for output in self.outputs])

        if isinstance(self.flag, int):
            witness_hex = "".join([self._get_witness_hex(witness) for witness in self.witness])
        else:
            witness_hex = ""

        lock_time_hex = self.lock_time.to_bytes(4, byteorder='little').hex()

        return version_hex + marker_hex + flag_hex + input_count_hex + input_hex + output_count_hex + output_hex + witness_hex + lock_time_hex

        
    def _get_input_hex(self, _input: Input) -> str:
        txid_hex = _input.txid.hex()
        vout_hex = _input.vout.to_bytes(4, byteorder='little').hex()
        script_sig_size_hex = self._get_hex_from_compact_size(_input.script_sig_size)
    ...
    def _get_output_hex(self, output: Output) -> str:
        ...
        script_pub_key_size_hex = self._get_hex_from_compact_size(output.script_pub_key_size)
    ...
    def _get_witness_hex(self, witness: Witness) -> str:
        stack_size_hex = self._get_hex_from_compact_size(len(witness.stack_items))
        stack_items_hex = "".join([
            self._get_hex_from_compact_size(stack_item.item_size) + stack_item.item.hex()
    ...
    def _get_compact_size(self, cur_pos: int) -> Tuple[int, int]:
        ...
        if self._cache_raw_bytes[cur_pos] <= 0xfc:
            ...
            return int.from_bytes(self._cache_raw_bytes[cur_pos+1:cur_pos+3], byteorder='little'), cur_pos + 3
        elif self._cache_raw_bytes[cur_pos] == 0xfe:
            return int.from_bytes(self._cache_raw_bytes[cur_pos+1:cur_pos+5], byteorder='little'), cur_pos + 5
        elif self._cache_raw_bytes[cur_pos] == 0xff:
            return int.from_bytes(self._cache_raw_bytes[cur_pos+1:cur_pos+9], byteorder='little'), cur_pos + 9
        else:
            raise ValueError("Invalid compact size first byte")
    
    def _get_input_size(self, _input: Input) -> int:
        return 32 + 4 + self._get_compact_size_size(_input.script_sig_size) + len(_input.script_sig) + 4
    
    def _get_output_size(self, output: Output) -> int:
        return 8 + self._get_compact_size_size(output.script_pub_key_size) + len(output.script_pub_key)
    
    def _get_witness_size(self, witness: Witness) -> int:
        return self._get_compact_size_size(len(witness.stack_items)) + sum([
            self._get_compact_size_size(stack_item.item_size) + len(stack_item.item)
            for stack_item in witness.stack_items
        ])
    ...
    def _get_transaction_size(self) -> int:
        ...
        if isinstance(self.flag, int):
            witness_size = sum(self._get_witness_size(witness) for witness in self.witness)
            marker_size = 1
            flag_size = 1
        else:
            witness_size = 0
    ...
    def print_noir_template(self) -> str:
        ...
        input_count_len = self._get_compact_size_size(self.input_count)
        input_size = sum(self._get_input_size(inp) for inp in self.inputs) + input_count_len
        output_count = self.output_count
        output_count_len = self._get_compact_size_size(self.output_count)
        output_size = sum(self._get_output_size(out) for out in self.outputs) + output_count_len
        if isinstance(self.flag, int):
            max_witness_stack_size = max(len(witness.stack_items) for witness in self.witness)
            witness_size = sum(self._get_witness_size(witness) for witness in self.witness)
        else:
    ...
    def get_outputs_from_inputs(self) -> tuple[list[int], list[Tuple[int, int, int]]]:
        outs = []
        pos = []
        cur_pos = 0
        txid = self.calculate_txid_from_hex(self.to_hex())
        response = requests.get(f"https://blockstream.info/api/tx/{txid}")
        if response.ok:
            tx_data = response.json()
            for inp in tx_data["vin"]:
                amount_bytes = inp["prevout"]["value"].to_bytes(8, byteorder='little')
                script_pub_key = bytearray.fromhex(inp["prevout"]["scriptpubkey"])
                script_pub_key_size = bytearray.fromhex(self._get_hex_from_compact_size(len(script_pub_key)))

                pos += [(cur_pos, cur_pos + 8, cur_pos + 8 + len(script_pub_key_size))]
    ...
    def calculate_txid_from_hex(self, raw_hex: str) -> str:
        tx = CTransaction.stream_deserialize(BytesIO(bytes.fromhex(raw_hex)))
        return b2x(tx.GetTxid()[::-1])

        
    def get_outputs_positions_as_toml(self, pos: list[Tuple[int, int, int]]) -> str:
        res = ""
